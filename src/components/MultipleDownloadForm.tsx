...
const MultipleDownloadForm: React.FC<MultipleDownloadFormProps> = ({ setMessage }) => {
  ...
  const generateCertificatePDF = async (certificate: Certificate): Promise<Blob> => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (failedCertIds.length > 0) {
            setSnackbarMessage(`Downloaded ${successCount} certificates. ${failedCertIds.length} failed.`);
            ...
            setSnackbarMessage(`Successfully downloaded all ${successCount} certificates!`);
  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
      <div className={styles.formGroup}>
        ...
        <div
          {...getRootProps()}
          className={`${styles.dropZone} ${isDragActive ? styles.active : styles.inactive}`}
        >
      ...
      {loading && (
        <div className={styles.progressContainer}>
          ...
          <p className={styles.progressText}>
            Processing {processedCertificates} of {totalCertificates} certificates ({progress}%)
          </p>
        </div>
      )}
